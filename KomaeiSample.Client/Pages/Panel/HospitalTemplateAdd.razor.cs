...
public partial class HospitalTemplateAdd
{
    [Parameter] public int ModelId { get; set; }
    private HospitalTemplateVm vm = new();
    ...
    private async Task Submit()
    {
        ...
        if (vm.File is not null)
        {
            ...
            content.Add(new StringContent(vm.TemplateName!), "TemplateName");
            content.Add(new StringContent(ModelId.ToString()), "ModelId");
            content.Add(new StreamContent(stream), "file", vm.File.Name);
        }
        var response = await http.PostAsync("HospitalTemplate/Add", content);
