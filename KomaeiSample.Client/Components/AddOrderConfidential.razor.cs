...
public partial class AddOrderConfidential
{
    [Parameter] public EnvelopeConfidentialDto[] EnvelopeConfidentials { get; set; } = Array.Empty<EnvelopeConfidentialDto>();
    ...
    protected override Task OnInitializedAsync()
    {
        ...
        Models = EnvelopeConfidentials.Where(x => x.ModelId == ModelId).OrderBy(x => x.ModelId).Select(x => new ModelDto { Id = x.ModelId, Title = x.ModelTitle!, FileName = x.ModelFileName, FileExtension = x.ModelFileExtension }).DistinctBy(x => x.Id).ToArray();
        vm.ModelId = ModelId;
        OnModelChange();
        StateHasChanged();
    ...
    private void OnModelChange()
    {
        ...
        Papers = EnvelopeConfidentials.Where(x => x.ModelId == vm.ModelId).Select(x => new PaperDto { Id = x.PaperId, Title = x.PaperTitle }).OrderBy(x => x.Title).DistinctBy(x => x.Id).ToArray();
    }
    ...
    private void OnPaperChange()
    {
        ...
        vm.CountId = null; Counts = null;
        vm.Price = null; vm.EnvelopeId = null;
        Grammages = EnvelopeConfidentials.Where(x => x.ModelId == vm.ModelId && x.PaperId == vm.PaperId).Select(x => new GrammageDto { Id = x.GrammageId, Title = x.GrammageTitle! }).OrderBy(x => int.Parse(x.Title!)).DistinctBy(x => x.Id).ToArray();
    }
    ...
    private void OnGrammageChange()
    {
        vm.CountId = null; Counts = null;
        vm.Price = null; vm.EnvelopeId = null;
        Counts = EnvelopeConfidentials.Where(x => x.ModelId == vm.ModelId && x.PaperId == vm.PaperId && x.GrammageId == vm.GrammageId).OrderBy(x => int.Parse(x.CountTitle!)).Select(x => new CountDto { Id = x.CountId, Title = x.CountTitle! }).DistinctBy(x => x.Id).ToArray();
    }
    ...
    private void OnCountChange()
    {
        try
        {
            var envelopeConfidential = EnvelopeConfidentials.Where(x => x.ModelId == vm.ModelId && x.PaperId == vm.PaperId && x.GrammageId == vm.GrammageId && x.CountId == vm.CountId).First();
            vm.EnvelopeId = envelopeConfidential.Id;
            vm.Price = envelopeConfidential.Price;
        }
        catch
        {
