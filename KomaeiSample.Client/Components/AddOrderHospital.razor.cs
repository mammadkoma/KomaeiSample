...
public partial class AddOrderHospital
{
    [Parameter] public EnvelopeHospitalDto[] EnvelopeHospitals { get; set; } = Array.Empty<EnvelopeHospitalDto>();
    ...
    protected override Task OnInitializedAsync()
    {
        ...
        Models = EnvelopeHospitals.Where(x => x.ModelId == ModelId).OrderBy(x => x.ModelId).Select(x => new ModelDto { Id = x.ModelId, Title = x.ModelTitle!, FileName = x.ModelFileName, FileExtension = x.ModelFileExtension }).DistinctBy(x => x.Id).ToArray();
        vm.ModelId = ModelId;
        OnModelChange();
        StateHasChanged();
    ...
    {
        vm.HospitalTemplateId = null; HospitalTemplates = null;
        ...
        HospitalTemplates = EnvelopeHospitals.Where(x => x.ModelId == vm.ModelId).Select(x => new HospitalTemplateDto { Id = x.HospitalTemplateId, TemplateName = x.HospitalTemplateTemplateName!, FileName = x.HospitalTemplateFileName! }).DistinctBy(x => x.Id).ToArray();
        HospitalTemplates = HospitalTemplates.OrderBy(x => x.Order).ToArray();
    }

    private void OnTemplateChange(int hospitalTemplateId)
    {
        vm.HospitalTemplateId = hospitalTemplateId;
        vm.PaperId = null; Papers = null;
        vm.GrammageId = null; Grammages = null;
        vm.CountId = null; Counts = null;
        vm.CellophaneId = null; Cellophanes = null;
        vm.UvId = null; Uvs = null;
        vm.Price = null; vm.EnvelopeId = null;
        Papers = EnvelopeHospitals.Where(x => x.ModelId == vm.ModelId && x.HospitalTemplateId == hospitalTemplateId).OrderBy(x => x.PaperTitle).Select(x => new PaperDto { Id = x.PaperId, Title = x.PaperTitle! }).DistinctBy(x => x.Id).ToArray();
    }
    ...
    private void OnPaperChange()
    {
        ...
        vm.UvId = null; Uvs = null;
        vm.Price = null; vm.EnvelopeId = null;
        Grammages = EnvelopeHospitals.Where(x => x.ModelId == vm.ModelId && x.HospitalTemplateId == vm.HospitalTemplateId && x.PaperId == vm.PaperId).Select(x => new GrammageDto { Id = x.GrammageId, Title = x.GrammageTitle! }).OrderBy(x => int.Parse(x.Title!)).DistinctBy(x => x.Id).ToArray();
    }
    ...
    private void OnGrammageChange()
    {
        ...
        vm.UvId = null; Uvs = null;
        vm.Price = null; vm.EnvelopeId = null;
        Counts = EnvelopeHospitals.Where(x => x.ModelId == vm.ModelId && x.HospitalTemplateId == vm.HospitalTemplateId && x.PaperId == vm.PaperId && x.GrammageId == vm.GrammageId).OrderBy(x => int.Parse(x.CountTitle!)).Select(x => new CountDto { Id = x.CountId, Title = x.CountTitle! }).DistinctBy(x => x.Id).ToArray();
    }
    ...
    private void OnCountChange()
    {
        ...
        vm.UvId = null; Uvs = null;
        vm.Price = null; vm.EnvelopeId = null;
        Cellophanes = EnvelopeHospitals.Where(x => x.ModelId == vm.ModelId && x.HospitalTemplateId == vm.HospitalTemplateId && x.PaperId == vm.PaperId && x.GrammageId == vm.GrammageId && x.CountId == vm.CountId).OrderBy(x => x.CellophaneTitle).Select(x => new CellophaneDto { Id = x.CellophaneId, Title = x.CellophaneTitle! }).DistinctBy(x => x.Id).ToArray();
    }
    ...
    private void OnCellophaneChange()
    {
        vm.UvId = null; Uvs = null;
        vm.Price = null; vm.EnvelopeId = null;
        Uvs = EnvelopeHospitals.Where(x => x.ModelId == vm.ModelId && x.HospitalTemplateId == vm.HospitalTemplateId && x.PaperId == vm.PaperId && x.GrammageId == vm.GrammageId && x.CountId == vm.CountId && x.CellophaneId == vm.CellophaneId).OrderBy(x => x.UvId).Select(x => new UvDto { Id = x.UvId, Title = x.UvTitle! }).DistinctBy(x => x.Id).ToArray();
    }
    ...
    private void OnUvChange()
    {
        try
        {
            var envelopeHospital = EnvelopeHospitals.Where(x => x.ModelId == vm.ModelId && x.HospitalTemplateId == vm.HospitalTemplateId && x.PaperId == vm.PaperId && x.GrammageId == vm.GrammageId && x.CountId == vm.CountId && x.CellophaneId == vm.CellophaneId && x.UvId == vm.UvId).First();
            vm.EnvelopeId = envelopeHospital.Id;
            vm.Price = envelopeHospital.Price;
        }
        catch
        {
