...
public class EnvelopeConfidentialController(EnvelopeConfidentialService envelopeConfidentialService) : AppController
{
    [HttpGet]
    public async Task<ActionResult> GetAll()
    {
        return Ok(await envelopeConfidentialService.GetAll());
    }
    ...
    {
        return Ok(await envelopeConfidentialService.GetAllEnables());
    }

    [HttpPost]
    [Authorize(Roles = $"{Constants.AdminRole},{Constants.SuperAdminRole}")]
    public async Task<ActionResult> Add(AddEditEnvelopeConfidentialVm vm)
    {
        return Ok(await envelopeConfidentialService.Add(vm));
    }

    [HttpPost]
    [Authorize(Roles = $"{Constants.AdminRole},{Constants.SuperAdminRole}")]
    public async Task<ActionResult> Edit(AddEditEnvelopeConfidentialVm vm)
    {
        return Ok(await envelopeConfidentialService.Edit(vm));
    }
    ...
    {
        var records = await envelopeConfidentialService.GetAllEnables();
        ...
        var pdfBytes = Document.Create(container =>
        {
            ...
            container.Page(page =>
            {
                ...
                page.Content().PaddingHorizontal(40).Column(col =>
                {
                    ...
                    col.Item().Text($"قیمت های {CategoriesEnum.Confidential.GetDescription()}").AlignCenter().FontColor("#1E6060").FontFamily("Yekan Bakh FaNum").FontSize(14);
                    col.Item().PaddingVertical(15).Table(table =>
                    {
                        ...
                        table.Header(header =>
                        {
                            ...
                            header.Cell().Border(1).AlignCenter().Text("نوع").FontFamily("Yekan Bakh FaNum").FontSize(12).Bold();
